...
DEFAULT_VIDEO = r"C:\Users\Nikita Vasilenko\Desktop\Ucheba\CV\HW-FaceDetection\shutterstock_13018979.mov"
#*********************************************************************************************************************************************MY MODEL**************************************************
#DEFAULT_MODEL = 'runs/detect/face_detection_yolov8/weights/best.pt'
#*********************************************************************************************************************************************MY MODEL**************************************************
DEFAULT_MODEL = r'C:\Users\Nikita Vasilenko\Desktop\Ucheba\CV\HW-FaceDetection\yolov8n-face.pt'
# Глобальные переменные для рисования линии
line_points = []
drawing = False
line_set = False
...
def draw_line_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Динамическое отображение линии во время рисования
...
def main():
    ...
    parser.add_argument('--line', type=int, nargs=4, help="Координаты линии x1 y1 x2 y2 (опционально)")
    ...
    # Читаем первый кадр для установки линии
    ret, first_frame = cap.read()
    if not ret:
        print("Не удалось прочитать первый кадр")
        return

    # Если линия передана через аргументы, используем её
    if args.line is not None:
        ...
        # Интерактивная установка линии мышкой
        ...
        while True:
            ...
            # Рисуем уже поставленные точки и линию
            ...
            if key == ord(' ') and line_set:
                break
            elif key == ord('q') or key == 27:  # q или ESC
                cap.release()
        ...
        # Перематываем видео на начало, чтобы обработать с первого кадра
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
    ...
    while True:
        ...
        if key == ord('q') or key == 27:  # q или ESC
            break
        # Проверка, не закрыто ли окно (работает не на всех системах, но попытка)
